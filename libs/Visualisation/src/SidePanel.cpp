    ...
    void SidePanel::AddSettings()
    {
        if (ImGui::CollapsingHeader("Settings", ImGuiTreeNodeFlags_DefaultOpen))
        {
            ...
            ImGui::SetNextWindowSize(
                ImVec2(displaySize.x * 1.0f / 3.0f, displaySize.y * 2.0f / 3.0f));
                    ...
                    "Modify simulation parameters", NULL, ImGuiWindowFlags_AlwaysAutoResize))
            {
                ...
                if (ImGui::InputInt("Number of columns",
                                    &_settingsState->temporaryParameters.columns))
                {
                }
                if (ImGui::InputInt("Expansion factor",
                                    &_settingsState->temporaryParameters.expansion))
                {
                }
